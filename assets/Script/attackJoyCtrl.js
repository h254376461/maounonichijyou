...
cc.Class({
    ...
    addAttackJoyCtrlTouchChangeListener: function(callback) {
        ...
        {                    
            ...
            if(distance < this.radius ) {
                if(distance>20){
                    ...
                    this.attackButton2.setPosition(touchPos);
                    //更新角度
                    var angle = this._getAngle(touchPos);
                    this._tanhuangBianxing(angle,distance); //更新弹簧
                }
                return true;
            }
        }
    ...
    onTouchMoved: function(touch, event) {
        ...
        if(this.radius >= distance){
            if(distance>20){
                ...
                this.node.opacity = 50
                //摇杆恢复位置
                this.attackButton2.setPosition(cc.p(0,0));
                this.curAngle = null;
                this._tanhuangBianxing(this.curAngle,0); //恢复弹簧
                //调用角度变化回调
                if(this.angleChange){
            ...
            this.attackButton2.setPosition(cc.p(x, y));
            //更新角度
            var angle = this._getAngle(touchPos);
            this._tanhuangBianxing(angle,distance); //更新弹簧
        }
    ...
    //弹簧变形  传入一个角度和距离
    _tanhuangBianxing(angle,dis){
        if(dis>this.radius){
            dis = this.radius;
        } 
        if(angle != null){
            // cc.log(angle);
            this.tanhuang.setRotation(270-angle);
        }else{
            this.tanhuang.setRotation(0);
        }
        let scaleNum = dis/150;     //弹簧长度未定义
        if(scaleNum !=0){
            this.tanhuang.setScale(1,scaleNum);
        }else{
            this.tanhuang.setScale(1,0.01);
        }
    },
